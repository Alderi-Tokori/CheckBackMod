...
function closeEnemiesTab() {
    document.getElementById("enemiesDiv").classList.add("hidden");
}
...
function displayEnemies() {
    ...
    for (i = 1; i < enemies.length; i++) {
        ...
        enemyBoxes[i - 1].addEventListener('mouseover', function () {
            if (game.enemies[parseInt(this.id)] > 0) {
                showEnemiesInfo(parseInt(this.id))
            }
        })
        enemyBoxes[i - 1].addEventListener('mouseout', function () {
            showEnemiesInfo(0)
        })
...
function showEnemiesInfo(x) {
    if (x == 0) {
        document.getElementById("enemiesInfo").innerHTML = ""
...
function closeFightingTab() {
    document.getElementById("fightingDiv").classList.add("hidden");
}
...
function displayEnemiesFightRarities(x) {
    if (x == 0) {
        document.getElementById("enemiesFightChances").innerHTML = ""
    } else if (x == 1) {
...
function attack() {
    ...
    setTimeout(() => {
        if (game.enemyHP <= 0) {
            fightRewards(enemies[enemiesChosen][4])
        } else if (game.currentHP <= 0) {
            alert("You died... and nothing happens. Remaining enemy hp: " + numberShort(game.enemyHP))
...
function fightRewards(x) {
    ...
    if (!game.enemies[enemiesChosen]) {
        game.enemies[enemiesChosen] = 1
    } else {
        game.enemies[enemiesChosen]++
    }
