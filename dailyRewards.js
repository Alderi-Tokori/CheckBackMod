function closeDailyRewardTab() {
    document.getElementById("dailyRewardDiv").classList.add("hidden");
}
...
function displayDailyRewards() {
    for (i = 0; i < 4; i++) {
        ...
        } else if (dailyRewardDay % 4 == 2) {
            document.getElementsByClassName("dayBox")[i].innerHTML += "<span class='crateName'><br>Skeletal crate</span><br><img src='img/crateSkeletal.png' style='width:4vh; margin: 0; margin-top: 1vh;'>"
        } else {
            document.getElementsByClassName("dayBox")[i].innerHTML += "<span class='crateName'><br>Ghost crate</span><br><img src='img/crateGhost.png' style='width:4vh; margin: 0; margin-top: 1vh;'>"
        }
    }
}
...
function displayDailyRewardRarities(x) {
    if (x == 0) {
        document.getElementById("dailyRewardRarities").innerHTML = ""
        ...
        document.getElementById("dailyRewardRarities").innerHTML = "Daily XP multipliers: <br>" + (17.5 + (game.dailyRewards + x) * 2.5) + " Base<br>" + DailyXPmultis()
    }
}
...
function DailyXPmultis() {
    result = "<br>"
    if (game.selectedPet >= 1) {
        result += "x" + numberShort(pets[game.selectedPet][1]) + " From pets<br>"
    }
    if (game.petsDiscovered >= 1) {
        result += "x" + numberShort(1 + game.petsDiscovered / 100) + " From pet collection<br>"
    }
    if (game.items[18] >= 1) {
        result += "x" + numberShort(game.XPBoostEffect ** 0.5) + " From XPBoost<br>"
    }
    if (game.items[18] >= 1) {
        result += "x" + numberShort(game.itemXP) + " From items<br>"
    }
